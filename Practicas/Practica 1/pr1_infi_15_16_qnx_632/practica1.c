...
void *acceso_pasillo(void *p)
{
     ...
     while(1) {
           printf("aceso_pasillo-%d: Esperando senal de peticion\n", id);
...
void crear_vehiculos(void) {  
  ...
  for(i=0; i<N_VEH; i++) {
      ...
      proc_veh[i] = fork();
      if(proc_veh[i] == 0) {
          execl("./veh1", "./veh1", arg1, arg1, arg3, NULL); 
...
id_grafi_t inic_graficos(void) {
   ...
   for(i=0; i<N_PULS; i++) {
      sprintf(nombre, "C%d", i); 
      crear_pulsador_codigo_id(FILA_PULS_CARGA, COL_PULS_CARGA+i, SIG_PULS_CARGA, nombre, i, idgr);
  }

   for(i=0; i<N_PULS; i++)  {    
      sprintf(nombre, "D%d", i); 
      crear_pulsador_codigo_id(FILA_PULS_DESCARGA, COL_PULS_DESCARGA+i, SIG_PULS_DESCARGA, nombre, i, idgr);
   }
